...
export function Projects({ locale, preview = false }: Readonly<{ locale?: string; preview?: boolean }>) {
  ...
  const tCopy = useTranslations("projectCopy");
    ...
    <section id="projects" aria-labelledby="projects-heading">
      ...
      <ul className="-mx-3 mt-8 flex flex-col gap-1" aria-label="Projects">
        {projects.slice(0, preview ? 3 : undefined).map((project) => {
          const name = tCopy(`${project.slug}.name`);
          const description = tCopy(`${project.slug}.description`);
          return (
            <li key={project.slug}>
              <a href={project.href} target="_blank" rel="noopener noreferrer" aria-label={`${name}: ${description}`} className="group flex cursor-pointer items-center gap-3 rounded-full px-3 py-2 transition-colors duration-200 hover:bg-muted active:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                <span className="min-w-0 shrink break-words text-sm text-foreground sm:truncate">{name}</span>
